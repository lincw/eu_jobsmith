...
//後端控制台共用資料層：右上角popover與「執行設定」面板都用這個hook。
...
export function useBackend(reloadKey = 0) {
  ...
  useEffect(() => {
    //初次載入後端清單；reloadexistawait之後才setState(non-rendering period synchronization), yeseffect正當用途。    // eslint-disable-next-line react-hooks/set-state-in-effect
    reload()
  }, [reloadKey])
  ...
  async function runTest(id: string) {
    ...
    try {
      ...
      if ((e as Error)?.name === "AbortError") {
        setTests((t) => ({ ...t, [id]: { ok: false, msg: "alreadyStop測試" } }))
        return
      }
...
// CLI代理清單（目前支援的本機CLI）。顯示名與後端id對應。export const CLI_AGENTS: { id: string; name: string }[] = [
